...
class NotionStatementGenerationService {
  ...
  private async generateStatementWithAI(
    ...
    const systemPrompt = `You are an expert facilitator helping to generate notion statements for deliberation discussions.
...
serve(async (req) => {
  ...
  try {
    EdgeLogger.info('Notion statement generation function called', { 
      method: req.method, 
      url: req.url 
    });

    const { deliberationId, context, statementType = 'summary' }: NotionStatementRequest = await parseAndValidateRequest(req, ['deliberationId', 'context']);
    const { supabase, openaiApiKey } = await validateAndGetEnvironment();

    EdgeLogger.info('Processing notion statement generation request', {
      deliberationId,
      statementType,
      contextLength: context.length
    });

    // Create notion statement generation service
    const statementService = new NotionStatementGenerationService(supabase, openaiApiKey);
    
    // Generate notion statement
    const result = await statementService.generateNotionStatement(deliberationId, context, statementType);

    EdgeLogger.info('Notion statement generation completed', {
      success: result.success,
      statementType: result.statementType,
      statementLength: result.statement?.length || 0
    });
