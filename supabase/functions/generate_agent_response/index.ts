...
class AgentResponseGenerationService {
  ...
  async generateAgentResponse(orchestrationResult, messageId, deliberationId, mode = 'chat') {
    ...
    try {
      ...
      if (!generatedContent) {
        // Insert fallback response when OpenAI returns no content
        const fallbackContent = `I apologize, but I'm experiencing a technical issue and cannot provide a complete response right now. Please try asking your question again, and I'll do my best to assist you.`;
        
        EdgeLogger.warn('OpenAI returned no content, inserting fallback response');
        
        // Insert fallback agent response
        const { data: agentMessage, error: insertError } = await this.supabase.from('messages').insert({
          ...
          agent_context: {
            ...
            model: selectedAgent.model,
            analysis: analysis,
            fallbackReason: 'OpenAI returned no content',
