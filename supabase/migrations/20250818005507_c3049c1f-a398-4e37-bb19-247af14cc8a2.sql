...
-- Function to get current user from access code system (create or replace)
...
-- Remove trigger that references auth.users (if it exists)
...
-- Create new policies for profiles table using access code system
CREATE POLICY "Access code users can view their own profile" ON public.profiles
  FOR SELECT USING (id = get_current_access_code_user());

CREATE POLICY "Access code users can update their own profile" ON public.profiles
  FOR UPDATE USING (id = get_current_access_code_user());

-- Create policies for messages table
CREATE POLICY "Access code users can view messages in their deliberations" ON public.messages
  FOR SELECT USING (
    deliberation_id IN (
...
CREATE POLICY "Access code users can create their own messages" ON public.messages
  FOR INSERT WITH CHECK (user_id = get_current_access_code_user()::text);

-- Create policies for ibis_nodes table
CREATE POLICY "Access code users can view IBIS nodes in their deliberations" ON public.ibis_nodes
  FOR SELECT USING (
    deliberation_id IN (
      SELECT deliberation_id FROM participants WHERE user_id = get_current_access_code_user()::text
    )
...
CREATE POLICY "Access code users can create IBIS nodes in their deliberations" ON public.ibis_nodes
...
CREATE POLICY "Access code users can view IBIS relationships in their deliberations" ON public.ibis_relationships
  FOR SELECT USING (
    deliberation_id IN (
      SELECT deliberation_id FROM participants WHERE user_id = get_current_access_code_user()::text
    )
...
CREATE POLICY "Access code users can create IBIS relationships in their deliberations" ON public.ibis_relationships
  FOR INSERT WITH CHECK (
    deliberation_id IN (
      SELECT deliberation_id FROM participants WHERE user_id = get_current_access_code_user()::text
    ) AND created_by = get_current_access_code_user()
...
CREATE POLICY "Access code users can view agent knowledge they created" ON public.agent_knowledge
  FOR SELECT USING (created_by = get_current_access_code_user());

CREATE POLICY "Access code users can create agent knowledge" ON public.agent_knowledge
  FOR INSERT WITH CHECK (created_by = get_current_access_code_user());

CREATE POLICY "Access code users can delete their own agent knowledge" ON public.agent_knowledge
  FOR DELETE USING (created_by = get_current_access_code_user());

-- Admin policies for all tables
CREATE POLICY "Access code admins can manage all profiles" ON public.profiles
  FOR ALL USING (
    EXISTS (
      SELECT 1 FROM access_codes 
      WHERE code = get_current_user_access_code() 
...
CREATE POLICY "Access code admins can manage all messages" ON public.messages
  FOR ALL USING (
    EXISTS (
      SELECT 1 FROM access_codes 
      WHERE code = get_current_user_access_code() 
...
CREATE POLICY "Access code admins can manage all IBIS nodes" ON public.ibis_nodes
  FOR ALL USING (
    EXISTS (
      SELECT 1 FROM access_codes 
      WHERE code = get_current_user_access_code() 
...
CREATE POLICY "Access code admins can manage all IBIS relationships" ON public.ibis_relationships
  FOR ALL USING (
    EXISTS (
      SELECT 1 FROM access_codes 
      WHERE code = get_current_user_access_code() 
...
CREATE POLICY "Access code admins can manage all agent knowledge" ON public.agent_knowledge
  FOR ALL USING (
    EXISTS (
      SELECT 1 FROM access_codes 
      WHERE code = get_current_user_access_code() 
