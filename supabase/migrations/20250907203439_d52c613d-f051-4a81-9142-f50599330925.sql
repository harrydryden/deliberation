-- Temporarily disable the trigger to avoid the updated_at error
DROP TRIGGER IF EXISTS update_prompt_templates_updated_at_trigger ON prompt_templates;
...
-- Re-create the trigger but only if updated_at column exists
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'prompt_templates' AND column_name = 'updated_at') THEN
