...
-- Step 8: Remove role column constraint that might reference user_roles
ALTER TABLE public.profiles DROP CONSTRAINT IF EXISTS chk_role_values;

-- Step 9: Add constraint for user_role column
ALTER TABLE public.profiles ADD CONSTRAINT chk_user_role_values 
CHECK (user_role IN ('user', 'admin', 'moderator'));

-- Step 10: Update RLS policies that might reference user_roles
-- Update any policies on other tables that might reference user_roles
...
-- Step 11: Drop user_roles table
DROP TABLE IF EXISTS public.user_roles CASCADE;

-- Step 12: Update any remaining database functions
...
-- Step 14: Update profiles RLS policies to use user_role column
-- The existing policies should continue to work, but let's ensure they reference the right column