-- Phase 2: Function consolidation with proper dependency handling

-- First, update RLS policies to use auth_is_admin() instead of is_admin()
-- Update deliberations table policies
...
-- Update storage policies to use auth_is_admin()
...
-- Now safely consolidate admin check functions
DROP FUNCTION IF EXISTS public.is_admin();
DROP FUNCTION IF EXISTS public.is_authenticated_admin();

-- Consolidate user participation functions - keep the most robust versions
