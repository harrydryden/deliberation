...
-- Remove the old elicit fields after copying to new share fields
UPDATE agent_configurations 
SET facilitator_config = (
  facilitator_config - 'ibis_facilitation' || 
  jsonb_build_object(
    'ibis_facilitation',
    (facilitator_config->'ibis_facilitation') 
    - 'elicit_issue_prompt' 
...
-- Update default prompts to focus on sharing existing content rather than eliciting new content
UPDATE agent_configurations 
SET facilitator_config = jsonb_set(
  jsonb_set(
    jsonb_set(
    ...
    '{ibis_facilitation,share_issue_prompt}',
    '"Based on the existing discussion, here are the key issues other participants have identified. Which of these resonates with your perspective?"'
  ),
  '{ibis_facilitation,share_position_prompt}',
  '"Other participants have taken various positions on this issue. Here are the main viewpoints that have been shared. Do any of these align with your thinking?"'
),
facilitator_config = jsonb_set(
  facilitator_config,
  '{ibis_facilitation,share_argument_prompt}',
  '"Here are the arguments other participants have made supporting different positions. Which of these do you find most compelling, or would you like to hear more about any particular argument?"'
)
WHERE agent_type = 'peer_agent' 
AND (facilitator_config->'ibis_facilitation'->>'share_issue_prompt' IS NULL 
     OR facilitator_config->'ibis_facilitation'->>'share_position_prompt' IS NULL 
     OR facilitator_config->'ibis_facilitation'->>'share_argument_prompt' IS NULL);