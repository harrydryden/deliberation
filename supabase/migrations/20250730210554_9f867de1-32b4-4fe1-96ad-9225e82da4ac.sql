-- Fix critical security vulnerabilities (with existing data handling)

-- 1. First, let's update existing short access codes to meet new requirements
UPDATE public.access_codes 
SET code = 
    CASE 
        WHEN length(code) < 12 THEN 
            code || SUBSTRING(MD5(RANDOM()::text) FROM 1 FOR (12 - length(code)))
        ELSE code
    END
...
-- 3. Fix role escalation vulnerability - strengthen RLS policy on profiles
...
-- 4. Create separate policy for admin role management
...
-- 6. Create trigger to log role changes and prevent escalation
...
-- 7. Now add strengthened access code constraints
ALTER TABLE public.access_codes ADD CONSTRAINT access_code_length_check 
CHECK (length(code) >= 12);

-- 8. Add failed authentication attempts tracking
