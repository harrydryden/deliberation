-- First, add created_at column to audit_logs if it doesn't exist
...
-- Add constraint to ensure user_id format is valid UUID (drop first if exists)
DO $$ 
BEGIN
  IF EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'valid_user_id_format') THEN
    ALTER TABLE messages DROP CONSTRAINT valid_user_id_format;
  END IF;
  ALTER TABLE messages ADD CONSTRAINT valid_user_id_format 
