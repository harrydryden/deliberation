...
const logger = createComponentLogger('OptimizedRealtimeService');
...
class OptimizedRealtimeService {
  ...
  subscribeToMessages(
    ...
    const channelName = `messages-${deliberationId || 'global'}`;
    ...
    if (!manager.isConnected && manager.subscriptionCount === 1) {
      manager.channel.subscribe((status) => {
        if (status === 'SUBSCRIBED') {
          manager.isConnected = true;
          manager.lastConnectTime = Date.now();
          logger.debug('Initial subscription successful', { channelName });
        }
      });
    }
  ...
  subscribeToAgentInteractions(
    ...
    manager.channel
      .on(
        ...
        (payload) => {
          if (payload.new && payload.new.message_type === 'agent') {
            callback(payload.new);
          }
        }
      );
