...
export const IBISNodeSchema = z.object({
  ...
  node_type: z.enum(NODE_TYPES as [string, ...string[]], {
...
export const IBISRelationshipSchema = z.object({
  ...
  relationship_type: z.enum(VALID_RELATIONSHIP_TYPES as [string, ...string[]], {
...
export const AIClassificationSchema = z.object({
  ...
  nodeType: z.enum(NODE_TYPES as [string, ...string[]]),
...
export const IBISSubmissionSchema = z.object({
  ...
  issueRecommendationRelationships: z.array(z.object({
    id: z.string().uuid(),
    type: z.enum(VALID_RELATIONSHIP_TYPES as [string, ...string[]]),
    confidence: z.number().min(0).max(1)
  })).default([]),
...
export const IBISFormSchema = z.object({
  ...
  nodeType: z.enum(NODE_TYPES as [string, ...string[]], {
    errorMap: () => ({ message: 'Please select a node type' })
  }),
