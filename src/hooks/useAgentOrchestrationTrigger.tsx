...
class CircuitBreaker {
  private static readonly CIRCUIT_BREAKER_ID = 'agent_response_generation';
...
class AgentResponseGenerationService {
  ...
  async generateAgentResponse(
    ...
    try {
      ...
      const { data: originalMessage, error: messageError } = await this.supabase
        .from('messages')
      ...
      const { data: agentMessage, error: insertError } = await this.supabase
        .from('messages')
        .insert({
          ...
          user_id: null, // Agent messages have null user_id
          agent_context: {
  ...
  private async fetchKnowledgeContext(query: string): Promise<string> {
    try {
      EdgeLogger.debug('Fetching knowledge context', { queryLength: query.length });
  ...
  private generateFallbackResponse(orchestrationResult: any, messageId: string, deliberationId: string): any {
  ...
  private generateErrorResponse(errorMessage: string, orchestrationResult: any, messageId: string, deliberationId: string): any {
    return {
      success: false,
      error: errorMessage,
      metadata: {
...
serve(async (req) => {
  ...
  try {
    ...
    const { orchestrationResult, messageId, deliberationId, mode = 'chat' }: GenerateResponseRequest = await parseAndValidateRequest(req, ['orchestrationResult', 'messageId', 'deliberationId']);
    ...
    const result = await responseService.generateAgentResponse(
      orchestrationResult,
      messageId,
      deliberationId,
      mode
