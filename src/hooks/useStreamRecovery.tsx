...
export const useStreamRecovery = (config: Partial<StreamRecoveryConfig> = {}) => {
  ...
  const addFailedMessage = useCallback((
    ...
    productionLogger.warn('Message added to recovery queue', {
      messageId,
      error,
      deliberationId
    });
  ...
  const removeFailedMessage = useCallback((messageId: string) => {
    ...
    productionLogger.debug('Message removed from recovery queue', { messageId });
  }, []);
  ...
  const scheduleRecovery = useCallback((
    ...
    const timeoutId = setTimeout(() => {
      productionLogger.info('Attempting automatic message recovery', {
        messageId,
    ...
    productionLogger.debug('Recovery scheduled for message', {
      messageId,
      delay,
      attemptCount: failedMessage.attemptCount
    });
  ...
  const clearAllFailures = useCallback(() => {
    ...
    productionLogger.info('All failed messages cleared from recovery queue');
  }, []);
