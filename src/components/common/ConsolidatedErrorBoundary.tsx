...
export class ConsolidatedErrorBoundary extends Component<Props, State> {
  ...
  componentDidCatch(error: Error, errorInfo: ErrorInfo) {
    ...
    if (process.env.NODE_ENV === 'development' && 'memory' in performance) {
      productionLogger.debug('Memory usage at error', (performance as any).memory);
    }
  ...
  handleRetry = () => {
    ...
    if (retryCount >= maxRetries) {
      productionLogger.warn('Max retry attempts reached', { errorId: this.state.errorId });
      return;
    }
    ...
    productionLogger.info('Retrying after error', { 
