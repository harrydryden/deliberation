...
export const AuthForm = () => {
  ...
  const handleAccessCodeAuth = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const {
        data: codeData,
        error: codeError
      } = await supabase.from("access_codes").select("id, code_type, is_used").eq("code", accessCode).maybeSingle();
  ...
  return <div className="min-h-screen flex items-center justify-center bg-deliberation-bg p-4">
      <Card className="w-full max-w-md">
        <CardHeader className="text-center">
          <CardTitle className="text-2xl font-bold text-democratic-blue">Deliberation</CardTitle>
          <CardDescription>Have your say, join the conversation</CardDescription>
        </CardHeader>
        <CardContent>
          <form onSubmit={handleAccessCodeAuth} className="space-y-4">
            <div className="space-y-2">
              <Label htmlFor="access-code">Access Code</Label>
              <Input id="access-code" type="text" placeholder="Enter your 10-digit access code" value={accessCode} onChange={e => {
              const value = e.target.value.replace(/\D/g, ''); // Only allow digits
              if (value.length <= 10) {
                setAccessCode(value);
              }
            }} maxLength={10} className="text-center tracking-widest font-mono text-lg" required />
              
            </div>
            <Button type="submit" className="w-full bg-democratic-blue hover:bg-democratic-blue/90" disabled={isLoading || accessCode.length !== 10}>
